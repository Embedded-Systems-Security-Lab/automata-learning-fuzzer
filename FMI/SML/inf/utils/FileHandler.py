...
from FMI.SML.inf.automata import MealyState, MealyMachine, OnfsmState, Onfsm, StochasticMealyMachine, StochasticMealyState, Mdp, MdpState

file_types = ['dot', 'png', 'svg', 'pdf', 'string']
automaton_types = {MealyMachine: 'mealy', Onfsm: 'onfsm', Mdp: 'mdp',
                   StochasticMealyMachine: 'smm'}
...
def _process_label(label, source, destination, automaton_type):
    ...
    if automaton_type == 'smm':
        inp = label.split('/', 1)[0]
        out_prob = label.split('/', 1)[1]
        temp = out_prob.rsplit(':', 1)
        out = temp[0]
        prob = temp[1]
        inp = int(inp) if inp.isdigit() else inp
        out = int(out) if out.isdigit() else out
...
def load_automaton_from_file(path, automaton_type, compute_prefixes=False):
    ...
    id_node_aut_map = { 'mealy': (MealyState, MealyMachine), 'mdp': (MdpState, Mdp),
                       'onfsm': (OnfsmState, Onfsm), 'smm': (StochasticMealyState, StochasticMealyMachine)}
